...
-- @author Darrell
-- #include <~/TI4-TTS/TI4/Helpers/TI4_DeckHelper>
...
local DECKS = {
    ...
    ['Secret Objectives'] = {
        ...
        cardNames = {
            ...
            ['Codices'] = {
                 'Fight with Precision Ω',
                 'Make an Example of Their World Ω',
                 'Turn Their Fleets to Dust Ω',
            }
        },
    },
    ['Agenda'] = {
        ...
        cardNames = {
            ['Base'] = {
                ...
                'Conventions of War',
                ['Core Mining'] = { removeForPoK = true },
                ['Demilitarized Zone'] = { removeForPoK = true },
                'Economic Equality',
                'Enforced Travel Ban',
                'Executive Sanctions',
                'Fleet Regulations',
                ['Holy Planet of Ixth'] = { score = true, removeForPoK = true },
                ...
                ['Representative Government'] = { removeForPoK = true },
                ['Research Team: Biotic'] = { removeForPoK = true },
                ['Research Team: Cybernetic'] = { removeForPoK = true },
                ['Research Team: Propulsion'] = { removeForPoK = true },
                ['Research Team: Warfare'] = { removeForPoK = true },
                ['Seed of an Empire'] = { score = true },
                ['Senate Sanctuary'] = { removeForPoK = true },
                ['Shard of the Throne'] = { score = true, removeForPoK = true },
                'Shared Research',
                'Swords to Plowshares',
                ['Terraforming Initiative'] = { removeForPoK = true },
                ['The Crown of Emphidia'] = { score = true, removeForPoK = true },
                ['The Crown of Thalnos'] = { removeForPoK = true },
	...
	['Genome'] = {
        transform = {
            parent = 'Splice Board',
            deck = {
                position = { x = -0.645688355, y = 0.26, z = -0.6959946 },
	...
	['Unit Upgrades'] = {
        transform = {
            parent = 'Splice Board',
            deck = {
                position = { x = -0.252987772, y = 0.26, z = -0.6959946 },
                rotation = { x = 0, y = 270, z = -0 },
            },
        },
		shuffleOnDiscard = true,
        cardNames = {
            ['TF'] = {
                ...
                'TF Prototype War Sun',
                'TF University War Sun',
                'TF The Dragon Freed',
                'TF Dawncrusher',
				'TF Exotrireme',
				'TF Dawncrusher',
				"TF Super-Dreadnought",
				'TF Corsair',
				'TF Ahk Syl Fier',
				'TF Saggitaria',
				'TF Strike Wing Alpha',
				'TF Exile',
				'TF Linkship',
				'TF Advanced Carrier',
				'TF Ambassador',
				...
				'TF Keeper Matrix',
				'TF Justicier Rail',
				'TF Production Biomes',
				'TF Floating Factories',
				'TF Helios Entity',
            },
        }
    },
	...
	['Abilities'] = {
        transform = {
            parent = 'Splice Board',
            deck = {
                position = { x = 0.139712781, y = 0.26, z = -0.6959946 },
                rotation = { x = 0, y = 270, z = -0 },
            },
        },
		shuffleOnDiscard = true,
        cardNames = {
            ['TF'] = {
				'TF Subatomic Splicer',
                "TF Bio-Synthetic Synergy",
                'TF Neural Parasite',
				'TF The Burning Eye',
				'TF Fleet Logistics',
				'TF Agency Supply Network',
				'TF Puppet Council',
				'TF Dimensional Tear',
				'TF Amalgamation',
				'TF Crucible',
				'TF Nanomachines',
				'TF Courier Transport',
				'TF Survival Instinct',
				'TF Genetic Research',
				'TF Radical Advancement',
				'TF Pacifist',
				'TF Entropic Harvest',
				'TF Terraform',
				'TF Temporal Command Suite',
				'TF Future Path',
				'TF Quantum Drive',
				'TF Stall Tactics',
				'TF Liberate',
				...
				'TF Indoctrination',
				'TF Peace Accords',
				'TF Nullification Field',
				'TF Instinct Training',
				'TF Lazax Gate Folding',
				'TF Hegemonic Trade Policy',
				'TF Fabrication',
				'TF Distant Suns',
				'TF Abundance',
				'TF Awaken',
				'TF Valkyrie Particle Weave',
				'TF Valkyrie Vanguard',
				"TF E-Res Siphons",
				'TF Singularity Y',
				'TF Singularity X',
				'TF Singularity Z',
				...
				'TF Harrow',
				'TF Assimilate',
				'TF Dimensional Splicer',
				'TF Reclamation',
				"TF Non-Euclidean Shielding",
				'TF Stymie',
				'TF Smothering Presence',
				'TF Slipstream',
				'TF Quantum Entanglementl',
				'TF Spec Ops Training',
				'TF Orbital Drop',
				'TF Quantum Datahub Node',
				'TF Star Forge',
				'TF Guild Ships',
				'TF Sled Factories',
				'TF Magmus Reactor',
				...
				'TF Nomadic',
				'TF Scavenge',
				'TF Munitions Reserves',
				'TF Armada',
				'TF Bioplasmosis',
				'TF Spatial Conduit Cylinder',
				'TF Mitosis',
				'TF Inheritance Systems',
				'TF Versatile',
				'TF Proxima Targeting VI',
				'TF Zealous',
				'TF Aerie Hololattice',
				'TF Supercharge',
				'TF Tactical Brilliance',
				'TF Unrelenting',
				'TF Quantum Entanglement',
            },
        }
    },
	...
	['Paradigm'] = {
        transform = {
            parent = 'Splice Board',
            deck = {
                position = { x = 0.6440563, y = 0.26, z = -0.6959946 },
                rotation = { x = 0, y = 270, z = -0 },
            },
        },
		shuffleOnDiscard = true,
        cardNames = {
            ['TF'] = {
                'TF Brood Swarm',
                'TF Brilliance of the Hylar',
                'TF Changing the Ways',
                'TF Gravitational Collapse',
				'TF Awakening',
				'TF Intelligence Unshackled',
				'TF Twilight Directive',
				'TF Event Horizon',
				'TF Opening The Eye',
				'TF Sins of the Father',
				'TF Insurrection',
				'TF Diaspora',
				'TF Artemris Ascendant',
				'TF Dimensional Reflection',
				"TF Eternity's End",
				'TF Witching Hour',
				'TF Limit Break',
				'TF Extortion',
				'TF Flock Migration',
				'TF The Laws Unwritten',
				'TF Devour World',
				'TF Void Transference',
				'TF Sanction of the Quieron',
				'TF Voice of the Council',
				'TF Poison of the Nefishh',
				'TF The Winds of Change',
				'TF Time Warp',
				'TF Blessing of the Yin',
				'TF Forge Legend',
				'TF The Lay of Lisis',
				'TF Overgrowth',
            },
...
--- Expose cards to remove from game for PoK.
function getRemoveForPoKCardNames()
    local result = {}
    local seen = {}
    _getCardAttributes('bogus')  -- make sure table exists
    for cardName, cardAttrs in pairs(_cardNameToCardAttributes) do
        if cardAttrs.removeForPoK and not seen[cardName] then
...
function injectCard(params)
    ...
    assert((not params.factionName) or type(params.factionName == 'string'), 'factionName')

    -- Make sure _cardNameToDeckAttributes exists.
    _getDeckAttributes('__bogus__')

    -- Link to deck, or to faction for those cards.
    local deckAttributes = params.deckName and _getDeckAttributesByType(params.deckName)
    _cardNameToDeckAttributes[params.cardName] = deckAttributes or {
        transform = {},
...
--- Add a homebrew deck.
function injectDeck(params)
    local deckAttributes = copyTable(params)
    _verifyDeck(params)

    DECKS[deckAttributes.deckName] = deckAttributes

    -- Card lists can be a mix of number->cardName and cardName->attributes entries.
    for source, cards in pairs(deckAttributes.cardNames or {}) do
        for _, cardName in ipairs(cards) do
            if type(cardName) == 'string' then
                injectCard({
                    cardName = cardName,
                    deckName = deckAttributes.deckName
                })
            end
        end
        for cardName, _ in pairs(cards) do
            if type(cardName) == 'string' then
                injectCard({
                    cardName = cardName,
                    deckName = deckAttributes.deckName
...
function _verifyDeck(deckAttributes)
    assert(type(deckAttributes.deckName) == 'string', 'deckName')
    assert(type(deckAttributes.transform) == 'table', 'transform')
    assert((not deckAttributes.transform.parent) or type(deckAttributes.transform.parent) == 'string', 'transform.parent')
    assert(type(deckAttributes.transform.deck) == 'table', 'transform')
    assert(type(deckAttributes.transform.deck.position) == 'table', 'transform.deck.position')
    assert(type(deckAttributes.transform.deck.position.x) == 'number', 'transform.deck.position.x')
    assert(type(deckAttributes.transform.deck.position.y) == 'number', 'transform.deck.position.y')
    assert(type(deckAttributes.transform.deck.position.z) == 'number', 'transform.deck.position.z')
    assert(type(deckAttributes.transform.deck.rotation) == 'table', 'transform.deck.rotation')
    assert(type(deckAttributes.transform.deck.rotation.x) == 'number', 'transform.deck.rotation.x')
    assert(type(deckAttributes.transform.deck.rotation.y) == 'number', 'transform.deck.rotation.y')
    assert(type(deckAttributes.transform.deck.rotation.z) == 'number', 'transform.deck.rotation.z')
    if deckAttributes.transform.discard then
        assert(type(deckAttributes.transform.discard) == 'table', 'transform.discard')
        assert(type(deckAttributes.transform.discard.position) == 'table', 'transform.discard.position')
        assert(type(deckAttributes.transform.discard.position.x) == 'number', 'transform.discard.position.x')
        assert(type(deckAttributes.transform.discard.position.y) == 'number', 'transform.discard.position.y')
        assert(type(deckAttributes.transform.discard.position.z) == 'number', 'transform.discard.position.z')
        assert(type(deckAttributes.transform.discard.rotation) == 'table', 'transform.discard.rotation')
        assert(type(deckAttributes.transform.discard.rotation.x) == 'number', 'transform.discard.rotation.x')
        assert(type(deckAttributes.transform.discard.rotation.y) == 'number', 'transform.discard.rotation.y')
        assert(type(deckAttributes.transform.discard.rotation.z) == 'number', 'transform.discard.rotation.z')
    end
    if deckAttributes.cardNames then
        assert(type(deckAttributes.cardNames) == 'table', 'deckAttributes.cardNames')
        for k, v in pairs(deckAttributes.cardNames) do
            assert(type(v) == 'table')
        end
    end
end

function _verifyDecks()
    for _, deck in pairs(DECKS) do
        print('checking ' .. deck.deckName)
        _verifyDeck(deck)
    end
    print('done')
end
...
function deal(params)
    ...
    for _, player in ipairs(Player.getPlayers()) do
        if player.getHandCount() > 0 then
            legalColorSet[player.color] = true
        end
    end
    if not legalColorSet[params.color] then
        printToColor('Unable to deal to players without a hand', params.color, 'Yellow')
        return
    end

    if params.count > 9 then
        printToColor('Attempted to deal too many cards, please wait a moment between number presses', params.color, 'Yellow')
        return
    end
...
function dealCoroutine()
    while #_dealQueue > 0 do
        ...
        while deck and deck.spawning do
            _dealCoroutineHeartbeat = Time.time
            coroutine.yield(0)
        end
...
function _dealHack(deck, count, playerColor)
    ...
    local rotation = {
        x = 0,
        y = zoneAttrs and zoneAttrs.rotation.y or 0,
        z = deck.getRotation().z
    }

    -- Take N cards NEAR the player.
    local cards = {}
    for i = 1, count do
    ...
    -- Wait for all cards to arrive, lock each card when done moving.
    while true do
        local busy = false
        for _, card in ipairs(cards) do
            if card.isSmoothMoving() then
                busy = true
            elseif not card.getLock() then
                card.setLock(true)
            end
        end
        if not busy then
    ...
    -- At this points cards have arrived, hanging in space locked.
    -- Process each card in turn.
    for _, card in ipairs(cards) do
        -- Grab json and card parameters.
        local spawnParams = {
            json              = card.getJSON(),
        ...
        -- Destroy original card, wait for TTS to actually delete.
        local guid = card.getGUID()
        card.addTag('DELETED_ITEMS_IGNORE')
        _deletePendingGuidSet[guid] = true
        destroyObject(card)
        while _deletePendingGuidSet[guid] do
            _dealCoroutineHeartbeat = Time.time
            coroutine.yield(0)
        end

        -- Spawn replacement copy.
        local clone = spawnObjectJSON(spawnParams)
        while clone.spawning do
            _dealCoroutineHeartbeat = Time.time
            coroutine.yield(0)
        end
...
-- Track top/bottom for agenda cards in the deck description.
function onStrategyCardPlayed(strategyCardGuid)
    local startegyCard = strategyCardGuid and getObjectFromGUID(strategyCardGuid)
    if startegyCard and startegyCard.getName() == 'Politics' then
        local agendaDeckGuid = getDeck('Agenda')
        local agendaDeck = agendaDeckGuid and getObjectFromGUID(agendaDeckGuid)
        if agendaDeck then
            agendaDeck.setDescription('Politics played')
        end
    end
end
...
local function _putAgendaCard(cardObject, onTop)
    ...
    -- Add to Agenda deck description what happened.
    local agendaDeckGuid = getDeck('Agenda')
    local agendaDeck = agendaDeckGuid and getObjectFromGUID(agendaDeckGuid)
    if agendaDeck then
        local desc = agendaDeck.getDescription()
        desc = desc .. '\nPut ' .. (onTop and 'top' or 'bottom')
        agendaDeck.setDescription(desc)
    end
end
...
function _isPurgableCard(cardName)
    assert(type(cardName) == 'string')
	local deckAttributes = _getDeckAttributes(cardName)
    if deckAttributes and deckAttributes.purge then
        return true
    end
...
_deletePendingGuidSet = {}
function onObjectDestroy(dying_object)
    _deletePendingGuidSet[dying_object.getGUID()] = nil
end
...
function resetPlanetCardNames()
    ...
    for planetName, tile in pairs(planetNameToTile) do
        ...
        elseif tile >= 500 then -- Thunder's Edge content uses 500+ tile numbers
            table.insert(planetSourceToNames.TE, planetName)
        else
...
function onLoad(saveState)
    ...
    -- Test methods.
    self.addContextMenuItem('Verify decks', _verifyDecks, false)
    ...
    -- Ensure Thunder's Edge (TE) planets like Ahk Creuxx are present in Planets deck
    do
        local deckAttributes = DECKS['Planets']
        if deckAttributes then
            deckAttributes.cardNames = deckAttributes.cardNames or {}
            deckAttributes.cardNames['TE'] = deckAttributes.cardNames['TE'] or {}
            local found = false
            for _, name in ipairs(deckAttributes.cardNames['TE']) do
                if name == 'Ahk Creuxx' then found = true break end
            end
            if not found then
                table.insert(deckAttributes.cardNames['TE'], 'Ahk Creuxx')
            end
        end
    end
    resetSnapPoints()
