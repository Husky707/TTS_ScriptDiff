...
-- #include <~/TI4-TTS/TI4/Helpers/TI4_SetupHelper>
...
local AllianceDeck, HS_Bag --Initialized onLoad
...
local SetupGeneric = {
    ...
    NAME_TO_TABLE_POSITIONS = {
        ...
        ['^x3 Commodities/Tradegoods$'] = {
            { x = -29.50, y = -0.19, z = -1.5, lock = true, dx = -5, dz = 6 },
            { x = 29.50, y = -0.19, z = 3, lock = true, dx = 5 },
            bagName = 'x3 Commodities/Tradegoods Bag', -- create bag around token
            bagColor = 'E6E42B',
        },
        ['^x1 Commodities/Tradegoods$'] = {
            { x = -29.50, y = -0.19, z = 1.5, lock = true, dx = -5, dz = 6 },
            { x = 29.50, y = -0.19, z = 6, lock = true, dx = 5 },
            bagName = 'x1 Commodities/Tradegoods Bag', -- create bag around token
            bagColor = 'E6E42B',
        },
        ['^x3 Fighter Token$'] = {
            { x = -26.50, y = -0.19, z = -1.5, lock = true, dx = -5, dz = 6 },
            { x = 26.50, y = -0.19, z = 3, lock = true, dx = 5 },
            bagName = 'x3 Fighter Tokens Bag', -- create bag around token
            bagColor = '1E87FF',
        },
        ['^x1 Fighter Token$'] = {
            { x = -26.50, y = -0.19, z = 1.5, lock = true, dx = -5, dz = 6 },
            { x = 26.50, y = -0.19, z = 6, lock = true, dx = 5 },
            bagName = 'x1 Fighter Tokens Bag', -- create bag around token
            bagColor = '1E87FF',
        },
        ['^x3 Infantry Token$'] = {
            { x = -23.50, y = -0.19, z = -1.5, lock = true, dx = -5, dz = 6 },
            { x = 23.50, y = -0.19, z = 3, lock = true, dx = 5 },
            bagName = 'x3 Infantry Tokens Bag', -- create bag around token
            bagColor = 'F3631C',
        },
        ['^x1 Infantry Token$'] = {
            { x = -23.50, y = -0.19, z = 1.5, lock = true, dx = -5, dz = 6 },
            { x = 23.50, y = -0.19, z = 6, lock = true, dx = 5 },
            bagName = 'x1 Infantry Tokens Bag', -- create bag around token
            bagColor = 'F3631C',
        },
    },
}
...
local SetupFaction = {
    NAME_TO_PLAYER_ZONE_OFFSET = {
        ...
        ['!TOKEN_NAME Planets Bag'] = { x = 6, y = 3, z = -6, unpack = true, dx = 2.5, snap = true, optional = true },
    },
...
local SetupPlayerTools = {
    NAME_TO_PLAYER_ZONE_OFFSET = {
        ['Technology Board'] = { x = 0, y = 3, z = 11 },
        ['Planet Board'] = { x = 0, y = 4, z = 11 },
        ['Build Area'] = { x = 0, y = 5, z = 11, flip = true },
    }
}
...
function getGameLengthSeconds()
    return _setupTime and math.max(os.time() - _setupTime, 0)
end
...
function onObjectDestroy(dyingObject)
    ...
    if guid and _putPendingGuidSet[guid] then
        _putPendingGuidSet[guid] = nil
    end
end
...
function _safeDestroyObject(object)
    assert(type(object) == 'userdata')
    object.setTags({'DELETED_ITEMS_IGNORE'})
...
local function offsetPosition(position, offset)
    ...
    local dir = position.z > 0 and 1 or -1
    if math.abs(position.z) < 1 then
        dir = 1 -- close enough to center, use "natural" direction
    end
    return {
        x = position.x + (offset.x or 0) * dir,
        y = position.y + (offset.y or 0),
        z = position.z + (offset.z or 0) * dir
    }
end
...
local function agendaCenter(extraRings)
    ...
    local useTF = Global.getVar('USE_TF') == true
    if not agendaPhaseMat then
        if useTF then
            -- Tolerate absence under TF; return a safe dummy center.
            return { x = 0, y = 0, z = 0 }
        end
...
--- Prepare scoreboard for game points use.
function SetupGeneric.scoreboard()
    debugLog('SetupGeneric.scoreboard')
    ...
    assert(scoreboard, 'missing scoreboard')

    local points = getGamePoints()
    local wantFaceDown = points > 10

    if scoreboard and scoreboard.is_face_down ~= wantFaceDown then
    ...
    local decals = {}
    if points > 10 and points < 14 then
        --local u = -(p.x - 3.25) / 6.5
        local x0 = (points - 6.5) / 15 * 6.5
        local x1 = 3.3
        local sheetColorJpg = 'https://steamusercontent-a.akamaihd.net/ugc/1499090241450210706/2DA6B757A764A6BC9F6051ABD64E6291135FBA0A/'
        table.insert(decals, {
            name = 'cover',
            url = sheetColorJpg,
            position = {
                x = (x0 + x1) / 2,
                y = -0.01, -- bottom is at 0 for model
                z = -0.02
            },
            rotation = {
                x = 270, -- facing down
            ...
            scale = { -- scale x,y is size in world units
                x = (x1 - x0),
                y = 0.98,
                z = 1
            }
...
function SetupGeneric.pack(setupBag)
    ...
    -- Remove bags created for tokens.
    local deleSet = {}
    for _, rule in pairs(SetupGeneric.NAME_TO_TABLE_POSITIONS) do
        if rule.bagName then
            deleSet[rule.bagName] = true
        end
    end
    ...
    for _, object in ipairs(getAllObjects()) do
        ...
        if deleSet[name] then
            table.insert(deleList, object)
        end
    end
    coroutine.yield(0)

    for _, object in ipairs(deleList) do
        _safeDestroyObject(object)
    end
    coroutine.yield(0)

    -- Pack only one copy of each object, delete the rest.
    local delayedPutObjects = {}
    for namePattern, objects in pairs(nameToObjects) do
        for i, object in ipairs(objects) do
            if i == 1 and not getEntryGuid(setupBag, namePattern) then
                object.setLock(false)
                table.insert(delayedPutObjects, object)
            else
                _safeDestroyObject(object)
            end
        end
    end
    coroutine.yield(0)  -- wait for unlock to finish
    coroutine.yield(0)

    for _, object in ipairs(delayedPutObjects) do
...
function SetupGeneric.unpack(setupBag, extraRings)
    ...
    for i, namePattern in ipairs(allNamePatterns(SetupGeneric)) do
        ...
        coroutine.yield(0)
        object.setLock(true)
        nameToLockedObject[namePattern] = object
    end
    local function isBusy()
        for _, object in pairs(nameToLockedObject) do
    ...
    coroutine.yield(0)
    local nameToJson = {}
    for namePattern, object in pairs(nameToLockedObject) do
        object.setLock(false)
    ...
    for _, zone in ipairs(zonesAttributes) do
        ...
        -- Skip per-player agenda items under TF
        if not useTF then
            ...
            print('[TF] USE_TF=true: skipping per-player agenda items for zone ' .. tostring(zone.color or '?'))
        end
    end

    -- Singleton agenda zone objects (For:/Against:/Votes/Speaker Token) -> skip under TF
    if not useTF then
        ...
        print('[TF] USE_TF=true: skipping Agenda-phase singletons (For:/Against:/Votes/Speaker Token).')
    end
    ...
    for namePattern, offsets in pairs(SetupGeneric.NAME_TO_TABLE_POSITIONS) do
        for _, offset in ipairs(offsets) do
            ...
            local callback = function(object) object.setLock(offset.lock or false) end

            if offsets.bagName then
                -- Special case for creating an infinite bag using this token.
                -- First create an infinite bag.
                ...
                bag.setName(offsets.bagName)
                bag.setColorTint(Color.fromHex('#' .. offsets.bagColor .. 'FF'))

                -- Adjust the token spawn to be above the bag, then add.
                local function newCallback(object)
                    bag.putObject(object)
                    bag.setLock(offset.lock or false)
                end
...
function SetupColor.removeHands()
    debugLog('SetupColor.removeAllHands')
...
function SetupColor.createHands()
    debugLog('SetupColor.createHands')

    local zonesAttributes = _zoneHelper.zonesAttributes()
    for i, zoneAttrs in ipairs(zonesAttributes) do
        local color = assert(SetupColor.DEFAULT_COLOR_LIST[i]) -- zoneAttrs.color not yet set

        local zoneCenter = zoneAttrs.center
        local position = offsetPosition(zoneCenter, SetupColor.HAND.OFFSET)
        local rotation = offsetRotation(zoneCenter, SetupColor.HAND.OFFSET)

        -- Objects moving through a hand (due to a "deal") no longer get pulled
        -- into hands (confirm?).  No need to offset the middle hand.

        local hand = spawnObject({
            type              = 'HandTrigger',
            position          = position,
            rotation          = rotation,
            scale             = SetupColor.HAND.SCALE,
            sound             = false,
            snap_to_grid      = false,
        })
        hand.setValue(color)
...
function SetupColor.unpack(color, colorBox, zone, agendaZoneCenter)
    ...
    -- TF-gate the per-player vote counter object ("!COLOR Player Votes")
    for namePattern, offset in pairs(SetupColor.NAME_TO_PLAYER_AGENDA_ZONE_OFFSET) do
        if not useTF then
            namePattern = string.gsub(namePattern, '!COLOR', color)
            local guid = assert(getEntryGuid(colorBox, namePattern), 'missing ' .. namePattern)
            ...
            -- Skip spawning "!COLOR Player Votes" when TF rules are on.
        end
    end
...
function SetupPlayerTools.unpack()
    -- Find the tools bag and any tools already on the table.
    local toolsBag = false
    local nameToJson = {}
    for _, object in ipairs(getAllObjects()) do
        local name = object.getName()
        if object.tag == 'Bag' and name == 'Tools and Helpers' then
            toolsBag = object
        end
        if SetupPlayerTools.NAME_TO_PLAYER_ZONE_OFFSET[name] then
            nameToJson[name] = object.getJSON()
        end
    end

    -- Get JSON for tools in the tools bag.
    for _, entry in ipairs(toolsBag and toolsBag.getObjects() or {}) do
        if SetupPlayerTools.NAME_TO_PLAYER_ZONE_OFFSET[entry.name] and not nameToJson[entry.name] then
            local object = toolsBag.takeObject({
                position          = toolsBag.getPosition() + vector(0, 5, 0),
                smooth            = false,
                guid              = entry.guid
            })
            ...
            nameToJson[entry.name] = object.getJSON()
            toolsBag.putObject(object)
            coroutine.yield(0)
        end
    end
    ...
    local zonesAttributes = _zoneHelper.zonesAttributes()
    for _, zone in ipairs(zonesAttributes) do
        for name, json in pairs(nameToJson) do
            local offset = SetupPlayerTools.NAME_TO_PLAYER_ZONE_OFFSET[name]
            spawnObjectJSON({
                json              = json,
                position          = offsetPosition(zone.center, offset),
                rotation          = zoneRotation(zone, offset),
...
function setup(params)
    ...
    assert(not params.playerTools or type(params.playerTools) == 'boolean')
    assert(not params.gamePoints or type(params.gamePoints) == 'number')
...
function setupCoroutine()
    ...
    local playerTools = params.playerTools
    ...
    if _zoneHelper.getPlayerCount() ~= playerCount or _zoneHelper.getExtraRings() ~= extraRings then
        ...
        SetupColor.removeHands()
        coroutine.yield(0)

        SetupColor.createHands()
        coroutine.yield(0)
...
function SetupFaction.unpackFactionBox(faction, zone)
    ...
    local otherBoxNameSet = {
        ...
        ['DS Factions'] = true,  -- Discordant Stars
        ['Discordant Stars Faction Pack'] = true,  -- Discordant Stars
    }
    for _, object in ipairs(getAllObjects()) do
...
function SetupFaction.unpack(faction, factionBox, zone, isFranken)
    ...
    for namePattern, offset in pairs(SetupFaction.NAME_TO_PLAYER_ZONE_OFFSET) do
        if (not isFranken) or offset.franken then
            ...
            local guid = getEntryGuid(factionBox, namePattern)
            assert(guid or offset.optional, 'missing ' .. namePattern)
            if guid then
                ...
                local object = factionBox.takeObject({
                    ...
                    smooth            = false,  -- if true infinite bags can glitch where hovering object is not above bag
                })
                nameToObject[object.getName()] = object
            end
        end
...
function SetupFaction.addLeaderSnapPoints(color)
...
function SetupFaction.unpackHomeSystemTile(faction, factionBox, homeSystemPosition, zone)
    ...
    local tileName = faction.offMapHome or (faction.tokenName:gsub('-', '%%-') .. ' Tile')

    if HS_Bag == nil then
        for _,each in ipairs(getAllObjects()) do
            if each.getName() == "Home System Tiles" then HS_Bag = each break end
        end
    end
    local guid = getEntryGuid(HS_Bag, tileName)
    local takeTarget = guid and HS_Bag or factionBox
    if not guid then guid = assert(getEntryGuid(factionBox, tileName), 'missing ' .. tileName) end
    local thisGUID = guid --could get overwritten by offMapHome before callback fires
    local tile = takeTarget.takeObject({
        guid              = guid,
        position          = homeSystemPosition,
        rotation          = rotation,
        smooth            = true,
        callback_function = function() _systemHelper.lockSystemTile(thisGUID) end,
    })

    local extraTile = false
    if faction.offMapHome then
        extraTile = tile
        tileName = faction.tokenName:gsub('-', '%%-') .. ' Tile'
        guid = getEntryGuid(HS_Bag, tileName)
        local takeTarget = guid and HS_Bag or factionBox
        if not guid then guid = assert(getEntryGuid(factionBox, tileName), 'missing ' .. tileName) end
        ...
        tile = takeTarget.takeObject({
            guid              = guid,
            position          = position,
            rotation          = rotation,
            smooth            = true,
            callback_function = function() _systemHelper.lockSystemTile(guid) end
        })
    end

    --HS tiles and planet cards are now stored on the table. Fetch the planets' cards now
    --The previous method of storing planet cards in a bag within the faction box is still supported (but the cards won't be placed by the 'TI4 Map Tool')
    local planetsDeck, abilityDeck = getObjectFromGUID('55c481'), getObjectFromGUID('b33eaf')
    if not planetsDeck then 
        for _,each in ipairs(getAllObjects()) do
            if each.getName() == "Planets" then planetsDeck = each break end
        end
    end
    if planetsDeck then
        if not abilityDeck then
            for _,each in ipairs(getAllObjects()) do
                if each.getName() == 'Legendary Abilities' then abilityDeck = each break end
            end
        end
        local cardNames = {}
        local legendCount = 0
        local fetchedCards = {}
        local function indexCards(systemData)
            if not systemData or not systemData.planets then return end
    
            for index,eachPlanet in ipairs(systemData.planets) do
                --preserve the index order of planets so legendary cards are unpacked next to/after their coresponding planet
                cardNames[eachPlanet.name] = index + legendCount
                if eachPlanet.legendaryCard then
                    legendCount = legendCount + 1
                    cardNames[eachPlanet.legendaryCard] = index + legendCount
                end
            end
        end
    
        indexCards(_systemHelper.systemFromGuid(guid))
        if faction.offMapHome then indexCards(_systemHelper.systemFromGuid(extraTile.getGUID())) end

        --Find the cards in the Planets and Legendary Planets decks
        local fetchData = {}
        if planetsDeck.type == "Card" then --This should *never really happen, but...
            if cardNames[planetsDeck.name] then table.insert(fetchedCards, {guid = planetsDeck.getGUID(), i = cardNames[planetsDeck.name], deck = planetsDeck}) end
        else --Look for planet ability cards here too, would be an easy mistake for decks to get merged
            for _,eachCard in ipairs(planetsDeck.getObjects()) do
                if cardNames[eachCard.name] then
                    table.insert(fetchData, {guid = eachCard.guid, i = cardNames[eachCard.name], deck = planetsDeck})
                    cardNames[eachCard.name] = nil
                    if not next(cardNames) then break end
                end
            end
            if abilityDeck and next(cardNames) then
                if abilityDeck.type == "Card" then
                    if cardNames[abilityDeck.name] then table.insert(fetchedCards, {guid = abilityDeck.getGUID(), i = cardNames[abilityDeck.name], deck = abilityDeck}) end
                else
                    for _,eachCard in ipairs(abilityDeck.getObjects()) do
                        if cardNames[eachCard.name] then
                            table.insert(fetchData, {guid = eachCard.guid, i = cardNames[eachCard.name], deck = abilityDeck})
                        end
                    end
                end
            end
        end
        --Place guids in the order they should be unpacked in
        local fetchOrder, count = {}, #fetchData
        for i = 1, count do
            local next = false
            for k,each in pairs(fetchData) do
                if not next or next.i > each.i then next = {i = each.i, k = k} end
            end
            if next then
                table.insert(fetchOrder, fetchData[next.k])
                fetchData[next.k] = nil
            end
        end
        --Take and place cards
        local offset = SetupFaction.NAME_TO_PLAYER_ZONE_OFFSET["!TOKEN_NAME Planets Bag"] 
        local nextOffset = offsetPosition(zone.center, offset)
        for _,each in ipairs(fetchOrder) do
            local rot = zoneRotation(zone, offset)
            if each.deck == abilityDeck then rot.y = rot.y - 90 end
            if each.deck.type == "Card" then
                if each.guid == each.deck.getGUID() then --If we even got here, this should be the case
                    each.deck.setPositionSmooth(nextOffset)
                    each.deck.setRotationSmooth(rot)
                    nextOffset = offsetPosition(nextOffset, {x = offset.dx, 0, 0})
                end
            else
                each.deck.takeObject({
                    guid = each.guid,
                    position = nextOffset,
                    rotation = rot,
                })
                nextOffset = offsetPosition(nextOffset, {x = -offset.dx, 0, 0})
                if each.deck.remainder then
                    each.deck = each.deck.remainder
...
function SetupFaction.unpackOther(faction, factionBox, zone)
    ...
    local function shouldUnpack(name)
        if string.match(name, 'Alliance$') and (not _includePoK) then
            return false
        end
        if string.match(name, '^' .. faction.tokenName:gsub('-', '%%-') .. ' .* Bag$') then
            return false
        end
        return true
    end
    local offset = {
        x = SetupFaction.OTHER_OFFSET.x or 0,
        y = SetupFaction.OTHER_OFFSET.y or 0,
        z = SetupFaction.OTHER_OFFSET.z or 0,
    }
    local _gotAlliance = false
    for _, entry in ipairs(factionBox.getObjects()) do
        if shouldUnpack(entry.name) then
            local rotation = zoneRotation(zone, offset)
            if string.match(entry.name, 'Alliance$') then
    ...
    --Alliance reference cards are now stored in their own deck for ease of access. Unpack this faction's now
    if not _gotAlliance and _includePoK then
        if AllianceDeck == nil then
            for _,each in ipairs(getAllObjects()) do
                if each.type == "Deck" and each.getName() == 'Alliance Ability Cards' then
                    AllianceDeck = each
                    break
                end
            end
        ...
        if AllianceDeck == nil then
            print("The Alliance Reference Deck is missing, could not fetch the ", faction.tokenName, " Alliance card.")
        else
            local function _isAllianceAbility(cardName)
                if not cardName then return false end
                --From what I can tell, Alliance ability cards don't have a strict naming convention
                --Could be .name, .shortName or .frankenName
                return cardName == (faction.tokenName.." Alliance") or cardName == (faction.shortName or "NIL").." Alliance" or cardName == (faction.frankenName or "NIL").." Alliance"
            end
            local rotation = zoneRotation(zone, offset)
            if AllianceDeck.type == "Card" and _isAllianceAbility(AllianceDeck.getName()) then
                AllianceDeck.setPositionSmooth(offsetPosition(zone.center, offset))
                AllianceDeck.setRotationSmooth(rotation)
                _gotAlliance = true
                AllianceDeck = nil
            else
                local objs = AllianceDeck.getObjects()
                for _,each in ipairs(objs) do
                    if _isAllianceAbility(each.name) then
                        _gotAlliance = true
                        AllianceDeck.takeObject({
                            guid = each.guid,
                            position = offsetPosition(zone.center, offset),
                            rotation = rotation
                        })
                        AllianceDeck = AllianceDeck.remainder or AllianceDeck --If all but the last card was taken
                    end
                end
            end
            if not _gotAlliance then
                print("Faction Unpacker could not find the ", faction.name,"'s Alliance ability card.")
            end
        end
    end
end
...
function SetupFaction.placeScoreCounters(ownerTokensBag, zone, zoneIndex, color)
    ...
    -- Actual clone has problems with the floating token above the bag.
    -- Instead spawn a new one, and add a token.
    local clone = spawnObject({
        type              = 'Infinite_Bag',
        position          = p,
        scale             = ownerTokensBag.getScale(),
        sound             = false,
        callback_function = function(object) object.setLock(true) end,
    })
    clone.setName(ownerTokensBag.getName())
    clone.setColorTint(Color.fromString(color))
    p.y = p.y + 3
    local token = ownerTokensBag.takeObject({
        position          = p,
        callback_function = function(object) clone.putObject(object) end,
        smooth            = true,
    })
    coroutine.yield(0)
    ...
    -- Place token on vote counter -> TF gated
    local useTF = Global.getVar('USE_TF') == true
    if not useTF then
        ...
        -- Skip placing a token on votes when TF rules are on.
    end
end
...
function unpackFactionCoroutine()
    ...
    if _includePoK then
        SetupFaction.addLeaderSnapPoints(color)
        coroutine.yield(0)
...
function SetupFranken.unpackStartingUnits(color)
    local startingUnitsFaction = false
    ...
    for _, object in ipairs(getAllObjects()) do
        ...
        if tokenName and _zoneHelper.zoneFromPosition(object.getPosition()) == color then
            if startingUnitsFaction then
                error('SetupFranken.unpackStartingUnits: multiple starting unit tokens for ' .. color)
            end
            startingUnitsFaction = _factionHelper.fromTokenName(tokenName)
        end
    ...
    if not startingUnitsFaction then
    ...
    -- Create a dummy faction table with the combination of starting units and flagship name.
    -- FactionHelper sould probably be overriding this but would need to remember original
    -- values and such.  This isn't too wacky given what franken is already doing.
    local myFaction = assert(_factionHelper.fromColor(color))
    local dummyFaction = {
        name = myFaction.name,
        tokenName = myFaction.tokenName,
        startingUnits = startingUnitsFaction.startingUnits,
        flagship = myFaction.flagship -- FactionHelper does set this for Franken
    }

    _safePrintToColor('SetupFranken.unpackStartingUnits: ' .. dummyFaction.tokenName .. ' starting units', color, color)
    SetupFaction.unpackUnits(color, dummyFaction, homeSystemTile)
end
...
function unpackFrankenCoroutine()
    ...
    if params.factionTokenName then
        ...
        local commandTokensBag = nameToObject[faction.tokenName .. ' Command Tokens Bag']
        local ownerTokensBag = nameToObject[faction.tokenName .. ' Owner Tokens Bag']
        factionSheet = nameToObject[faction.name .. ' Sheet']
        nameToObject = nil
        while commandTokensBag.spawning or ownerTokensBag.spawning do
            coroutine.yield(0)
        end
        ...
        factionSheet = faction and getObjectFromGUID(faction.factionSheetGuid)
    end
...
function onLoad(saveState)
    ...
    self.addContextMenuItem('Fix unit scale', fixUnitScale)

    local function testScoreboard()
        setGamePoints(11)
        SetupGeneric.scoreboard()
    end
    self.addContextMenuItem('Test 11 points', testScoreboard)

    AllianceDeck = getObjectFromGUID("146599")
    HS_Bag = getObjectFromGUID("c94fbe")
end
