...
local UNIT_TYPES = {
    ...
    'Mech',
    'PDS',
	'Space Dock',  -- WHY? WHY EXCLUDE? Took me so long to find why the unit was being excluded.
}
...
function createUi()
    ...
    if not _allPossibleUnitAttrs then
        ...
        _allPossibleUnitAttrs['Fighter']['groundCombat'] = {}  -- naulo flagship
...
function generateRollerXmlTable(unit_data)
    ...
    for _, unit_id in ipairs(UNIT_TYPES) do
		-- Keep the old look while having Space Dock capatable roller
		if unit_id ~= "Space Dock" then
...
function generateUnitAbility(unit_id, ability_id, ability_details)
    ...
    if type(ability_details) == 'table' then
        onClickValue = "onClickUnitAbility"
    else
...
function onClickUnitAbility(player, value, id)
    if (not player.seated) or player.color == 'Grey' then
        return
    elseif value == '-3' then
        return  -- middle button click
    end
    local unitType, abilityId = string.match(id, "^(.+)%.ability%.(.+)$")
    rollDiceForAbility(player, abilityId)
end
...
function rollDiceForAbility(player, ability)
    ...
    for unit_id, unit_stats in pairs(unitData) do
        ...
        if unit_count > 0 and unit_stats[ability] then
			...
			if _galvanizeDice then
				for unit_type, galvanizeCount in pairs(_galvanizeDice) do
					if unit_id == unit_type then
						abilityAttrs.extraDice = (abilityAttrs.extraDice or 0) + galvanizeCount
					end
				end
			end
            Roller.addDice({
				...
				x89Bonus = abilityAttrs.x89Bonus,
...
function inject(params)
	...
	assert((not params.galvanizeDice) or type(params.galvanizeDice) == 'table')
	...
	_galvanizeDice = params.galvanizeDice or false
