...
-- @author Darrell
-- #include <~/TI4-TTS/TI4/Helpers/TI4_GameDataHelper>
...
local MIN_PERIODIC_SECONDS = 30
local MAX_PERIODIC_SECONDS = 3600
local DEFAULT_KEY_PERIODIC_SECONDS = 45
...
local _anonymizedTimerId = false
local _streamerTimerId = false
...
--- Expose method so outsiders can trigger anonymized data update.
function triggerUploadGameData()
    ...
    local function wrappedUploadGameData()
        ...
        _uploadAnonymizedGameData()
        --_uploadStreamerGameData()
        --_uploadAnalytics()
    end
...
function asyncGatherDataCoroutine()
    ...
    result.mapString = _state.mapString or ''
    coroutine.yield(0)
    coroutine.yield(0)
    ...
    -- Include per-round history of game?
    ...
    if callbackObject then
        callbackObject.call(params.callbackFunctionName, {
            params = params,
            data = result
        })
    end
...
function HexSummary.sortResults(results)
    local function soryBy(resultA, resultB)
        local a = resultA.tile or 0
        local b = resultB.tile or 0
        if a < b then return true end
        if a > b then return false end

        local a = resultA.planetIndex or 0
        local b = resultB.planetIndex or 0
        if a < b then return true end
        if a > b then return false end

        -- Attachments are always last.
        local a = resultA.attachment and 1 or 0
        local b = resultB.attachment and 1 or 0
        if a < b then return true end
        if a > b then return false end

        local a = resultA.color or ''
        local b = resultB.color or ''
        if a < b then return true end
        if a > b then return false end

        local a = resultA.token and 1 or 0
        local b = resultB.token and 1 or 0
        if a < b then return true end
        if a > b then return false end

        local a = resultA.count or 1
        local b = resultB.count or 1
        if a < b then return true end
        if a > b then return false end

        -- Arbitrary but consistent final tie breaker.
        local a = resultA.code or ''
        local b = resultB.code or ''
        return a < b
    end
...
function HexSummary.encode(hexToSystem, results)
    ...
    for _, result in ipairs(results) do
        ...
        if tile ~= stickyTile then
            if stickyTile then
                table.insert(encoded, HexSummary.delimiter.system)
            end
            table.insert(encoded, assert(tileToEncoded[tile]))
            stickyTile = tile
            stickyPlanetIndex = 0
            stickyColor = ''
            stickyCount = 1
        ...
        local planetIndex = result.planetIndex or 0
        if planetIndex ~= stickyPlanetIndex then
        ...
        -- Attachment change?
        local attachment = result.attachment or false
        if attachment ~= stickyAttachment then
            -- Should only ever toggle to true.  Provide some extra info vs assert.
            if not attachment then
                error('error with attachment on tile ' .. tile)
            end
        ...
        -- Color change?
        local color = result.color or ''
        if color ~= stickyColor then
            table.insert(encoded, HexSummary.colorToCode[color])
            stickyColor = color
            stickyCount = 1
        end
        ...
        if count ~= stickyCount then
            table.insert(encoded, count)
            stickyCount = count
        end

        local code = result.code or ''
...
function Cards.sumAttrs(cards)
    ...
    for _, card in ipairs(cards) do
        ...
        if attrs then
            ...
            if attrs.trait then
                if type(attrs.trait) == "table" then
                    for _,each in ipairs(attrs.trait) do
                        sum.traits[each] = (sum.traits[each] or 0) + 1
                    end
                else
                    sum.traits[attrs.trait] = (sum.traits[attrs.trait] or 0) + 1
                end
            end
            if attrs.tech then
...
function Score.getScoreboardPoints()
    return _setupHelper.getGamePoints()
end
...
function Map.getMapString()
    ...
    if not buildList then
        print('Gamedata: error reading the map string, it might be invalid.  Ignoring.')
        return false
    end
...
function _doNothingCallback()
end

function onPlaceTradeGoodsAndSetTurns()
    -- Update data to capture start-of-round state.  Do not upload this data,
    -- wait for the timer to go off.
    if _state._gamedataOptIn or _gameDataKey then
        asyncGatherData({
            guid = self.getGUID(),
            includeSteamNames = false,
            includeHistory = true,
            callbackFunctionName = '_doNothingCallback',
        })
    end
end

function onPlayerTurnStart(player_color_start, player_color_previous)
    if (_state._gamedataOptIn or _gameDataKey) and (not _state.mapString) then
...
function startPeriodicUpdates(params)
    ...
    -- The params.delay is associated with streamer gamedata.
    ...
    if _anonymizedTimerId then
        Timer.destroy(_anonymizedTimerId)
        _anonymizedTimerId = false
    end
    _anonymizedTimerId = self.getGUID() .. '_anonymizedTimerId'
    Timer.create({
        identifier = _anonymizedTimerId,
        function_name = '_uploadAnonymizedGameData',
        function_owner = self,
        delay = DEFAULT_ANON_PERIODIC_SECONDS,
        repetitions = 0,  -- until stopped
    })
    _uploadAnonymizedGameData()  -- do not wait for first timer, upload one now

    if _streamerTimerId then
        Timer.destroy(_streamerTimerId)
        _streamerTimerId = false
    end
    _streamerTimerId = self.getGUID() .. '_streamerTimerId'
    Timer.create({
        identifier = _streamerTimerId,
        function_name = '_uploadStreamerGameData',
        function_owner = self,
        delay = delay,
        repetitions = 0,  -- until stopped
    })
    _uploadStreamerGameData()  -- do not wait for first timer, upload one now
    ...
    _analyticsTimerId = self.getGUID() .. '_analyticsTimerId'
    Timer.create({
...
function stopPeriodicUpdates()
    -- Only stop the streamer updates.
    _gameDataKey = false
    if _streamerTimerId then
        Timer.destroy(_streamerTimerId)
        _streamerTimerId = false
...
function _shouldUploadAnonymized()
    if not _state._gamedataOptIn then
        return false
    end
    ...
    -- multiple players before uploading vonultary anonymized game data.
    local seated = getSeatedPlayers() or {}
    return #seated > 1
end

function _uploadAnonymizedGameData()
    if not _shouldUploadAnonymized() then
        return
    end
    asyncGatherData({
        guid = self.getGUID(),
        ...
        crcKey = 'anonymized',
        urlPath = '/posttimestamp',
        urlArgs = {
            timestamp = _state.setupTimestamp or 0,
        },
    })
end

function _uploadStreamerGameData()
    ...
    asyncGatherData({
        guid = self.getGUID(),
        includeSteamNames = true,
        includeHistory = false,
        callbackFunctionName = '_uploadGameDataCallback',
        crcKey = 'streamer',
        urlPath = '/postkey',
        urlArgs = {
            key = _gameDataKey, -- CAREFUL, passing key means record is ONLY in cache
            timestamp = _state.setupTimestamp or 0,
        },
    })
end

function _uploadGameDataCallback(result)
    local params = assert(result.params)
    local data = assert(result.data)

    -- Suppress updates if nothing (other than timestamp) changed.
    if params.crcKey and data.crc then
        if data.crc == _lastCrc[params.crcKey] then
            return
        end
        _lastCrc[params.crcKey] = data.crc
    end
    ...
    local path = params.urlPath or '/data'

    local args = {}
    for k, v in pairs(params.urlArgs or {}) do
        table.insert(args, k .. '=' .. v)
    end
    args = table.concat(args, '&')

    local url = host .. path .. '?' .. args

    local method = 'POST'
    local download = true -- handle the response text
    ...
    WebRequest.custom(url, method, download, body, headers, callback)
end
