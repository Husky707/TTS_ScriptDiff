...
--Cards can also be given the "DO_NOT_READY" tag
...
function dealAllActionCards(dealOrder)
    ...
    local function processColor(color)
		...
		if abilitiesSet['TF Inheritance Systems'] then
            count = count + 1
            _safeBroadcastToColor('Inheritance Systems active.', color, {0.1,0.5,0.1})
        end
...
function _allocateCommandTokens()
    ...
    local function processColor(color)
		...
		if abilitiesSet['TF Versatile'] then
            count = count + 1
            _safeBroadcastToColor('Versatile active.', color, {0.1,0.5,0.1})
        end
		if abilitiesSet['TF Inheritance Systems'] then
...
function _readyCards()
    ...
    local function isCandidate(object)
        ...
        if object.hasTag("DO_NOT_READY") then
            return false
        end
